...
int main(int argc, char* argv[]) {
  ...
  Values initial;
  initial.insert(X(1),
      Pose3(Rot3(0.8, 0, 0, 0, -1.2, 0, 0, 0, -0.3), Point3(0, 0, 2)));

  /* 4. Optimize the graph using Levenberg-Marquardt*/
  Values LMresult = LevenbergMarquardtOptimizer(graph, initial).optimize();
  LMresult.print("Final LMresult:\n");

    //求解
    GaussNewtonOptimizer optimizer(graph, initial);
    Values GNresult = optimizer.optimize();
    GNresult.print("Final GNresult:\n");
  return 0;
}
