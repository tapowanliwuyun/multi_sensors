...
public:
  ...
  // 使用NoiseModelFactorN基类，必须重写两个函数
  // 第一个是“evaluaterror”函数。此函数实现所需的测量函数，在以提供的变量值进行评估时返回误差向量
  // 如果需要，还必须计算该测量函数的雅可比
  Vector evaluateError(const Pose2& q, boost::optional<Matrix&> H = boost::none) const
  {
    ...
    // 关于这个问题，可以查看gtsam.pdf的p9推导，但是这里不同的gtsam版本是有所更新的，当前的版本应该是比较老的，所以可以查看在自己的《31 gtsam研究》的图片补充
    // 或者直接看本章节 王泽霖介绍的
  ...
  // work fine.
  // 第二个是允许复制因子的“克隆”函数。在大多数情况下使用默认的赋值构造函数即可
