...
// 当前激光帧点云信息，包括的历史数据有：运动畸变校正，点云数据，初始位姿，姿态角，有效点云数据，角点点云，平面点点云等
    ...
    //接收 imageProjection.cpp 中发布的去畸变的点云，实时处理的回调函数
    ...
    void calculateSmoothness()
    {
        ...
        {
            // 用当前激光点前后5个点计算当前点的曲率
            //注意 ， 这里把前后五个点共 10 个点加起来，还减去了 10 倍的当前点
            ...
            //1表示角点，-1表示平面点,0  默认是 less corner
            cloudLabel[i] = 0;
 
            // 存储该点曲率值、激光点一维索引
            //之所以可以这样操作，是因为在 initializationValue 部分，对 cloudSmoothness 进行过初始化，
            //否则直接对 cloudSmoothness[i] 赋值，一定会报段错误
    ...
    void markOccludedPoints()
    {
        ...
        for (int i = 5; i < cloudSize - 6; ++i)
        {
            ...
            // 两个点在同一扫描线上，且距离相差大于 0.3，认为存在遮挡关系
