...
public:
    ...
    mapOptimization()
    {
        ...
        // 发布激光里程计，它与上面的激光里程计基本一样，只是 roll 、 pitch 用 imu 数据加权平均了一下，z做了限制
    ...
    */
    ...
    void laserCloudInfoHandler(const lio_sam::cloud_infoConstPtr& msgIn)
    {
        ...
        if (timeLaserInfoCur - timeLastProcessing >= mappingProcessInterval)
        {
            ...
            // 当前帧位姿初始化
            // 1、 如果是第一帧，用原始 imu 数据的 RPY 初始化当前帧位姿（旋转部分）
    ...
    */
    void updateInitialGuess()
    {
        ...
        // 前一帧的位姿， 注：这里指 lidar 的位姿，后面都简写成位姿
        ...
        // odomAvailable 和 imuAvailable 均来源于 imageProjection.cpp 中赋值，
        // imuAvailable 是遍历激光帧前后起止时刻0.01s之内的imu数据，
        //如果都没有那就是 false ，因为imu频率一般比激光帧快，因此这里应该是都有的。
        // odomAvailable 同理，是监听imu里程计的位姿，如果没有紧挨着激光帧的imu里程计数据，那么就是false；
        //这俩应该一般都有
        if (cloudInfo.odomAvailable == true)
        {
            //  cloudInfo 来自 featureExtraction.cpp 发布的 lio_sam/feature/cloud_info ,
            //而其中的 initialGuessX 等信息本质上来源于 ImageProjection.cpp 发布的 deskew/cloud_info 信息，
            //而 deskew/cloud_info 中的 initialGuessX 则来源于 ImageProjection.cpp 中的回调函数 odometryHandler ，
            // odometryHandler 订阅的是 imuPreintegration.cpp 发布的 odometry/imu_incremental 话题，
            ...
            {
                // 赋值给前一帧
                // lastImuPreTransAvailable 是一个静态变量，初始被设置为false,之后就变成了true
                //也就是说这段只调用一次，就是初始时，把imu位姿赋值给lastImuPreTransformation
                lastImuPreTransformation = transBack;
                lastImuPreTransAvailable = true;
            } else {
                // 当前帧相对于前一帧的位姿变换，imu里程计计算得到
                // lastImuPreTransformation 就是上一帧激光时刻的imu位姿, transBack 是这一帧时刻的imu位姿
                //求完逆相乘以后才是增量，绝不可把 imu_incremental 发布的当成是两激光间的增量
    ...
    */
    ...
    void extractNearby()
    {
        ...
        //指定半径范围查找近邻
        //球状固定距离半径近邻搜索
        // surroundingKeyframeSearchRadius 是搜索半径， pointSearchInd 应该是返回的 index ， pointSearchSqDis 应该是依次距离中心点的距离
        ...
        for(auto& pt : surroundingKeyPosesDS->points)
        {
            ...
            //注意，这里的 intensity 应该不是强度，因为在函数 saveKeyFramesAndFactor 中:
            // thisPose3D.intensity = cloudKeyPoses3D->size();
            //就是索引，只不过这里借用 intensity 结构来存放
        ...
        // 把10s内的关键帧也加到 surroundingKeyPosesDS 中,注意是“也”，原先已经装了下采样的位姿(位置)
        ...
        //对降采样后的点云进行提取出边缘点和平面点对应的 localmap 
        extractCloud(surroundingKeyPosesDS);
    }
 
    /**
         * 将相邻关键帧集合对应的角点、平面点，加入到局部map中，作为 scan-to-map 匹配的局部点云地图
    */
    ...
    void scan2MapOptimization()
    {
        ...
        if (laserCloudCornerLastDSNum > edgeFeatureMinValidNum && laserCloudSurfLastDSNum > surfFeatureMinValidNum)
        {
            ...
            //使用了9轴imu的 orientation 与做 transformTobeMapped 插值，并且roll和pitch收到常量阈值约束（权重）
            transformUpdate();
        } else {
    ...
    */
     ...
     * 3、执行因子图优化
     * 4、得到当前帧优化后位姿，位姿协方差
     * 5、添加cloudKeyPoses3D，cloudKeyPoses6D，更新 transformTobeMapped，添加当前关键帧的角点、平面点集合
    */    
    ...
    */
    bool saveFrame()
    {
        if (cloudKeyPoses3D->points.empty())
            return true;
        // 前一帧位姿
        //注：最开始没有的时候，在函数 extractCloud 里面有
    ...
    */
    void performLoopClosure()
    {
        ...
        *copy_cloudKeyPoses6D = *cloudKeyPoses6D;
        ...
        {
            // 提取当前关键帧特征点集合，降采样;
            //loopFindNearKeyframes 形参分别为点云集合，当前帧的索引，搜索半径
    ...
    */
    bool detectLoopClosureDistance(int *latestID, int *closestID)
    {
        ...
        //配置文件中默认 historyKeyframeSearchRadius =15m
