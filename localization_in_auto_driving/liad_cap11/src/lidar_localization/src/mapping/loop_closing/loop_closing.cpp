...
bool LoopClosing::CloudRegistration(int key_frame_index) {
    ...
    // 计算相对位姿
    current_loop_pose_.pose = map_pose.inverse() * result_pose;// Tlm_cl = Tlm_m * Tm_cl;   lm局部地图；  cl当前点云；
...
bool LoopClosing::JointMap(int key_frame_index, CloudData::CLOUD_PTR& map_cloud_ptr, Eigen::Matrix4f& map_pose) {
    ...
    // 合成地图
    Eigen::Matrix4f pose_to_gnss = map_pose * all_key_frames_.at(key_frame_index).pose.inverse();//T = Tm_gl * Tssl_m
    ...
    for (int i = key_frame_index - extend_frame_num_; i < key_frame_index + extend_frame_num_; ++i) {
        ...
        Eigen::Matrix4f cloud_pose = pose_to_gnss * all_key_frames_.at(i).pose;//Tm_gl
