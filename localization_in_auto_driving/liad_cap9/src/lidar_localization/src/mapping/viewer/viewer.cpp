...
bool Viewer::Update(std::deque<KeyFrame>& new_key_frames,
                    ...
                    PoseData transformed_data,//Tm_l
    ...
    optimized_odom_ = transformed_data;//Tm_l
    //optimized_odom_.pose = pose_to_optimize_ * optimized_odom_.pose;
    optimized_odom_.pose = optimized_odom_.pose * pose_to_optimize_;//Tm_lop = Tm_l * Tl_lop
...
bool Viewer::OptimizeKeyFrames() {
    ...
    while (optimized_index < optimized_key_frames_.size() && all_index < all_key_frames_.size()) {
        if (optimized_key_frames_.at(optimized_index).index < all_key_frames_.at(all_index).index) {
            ...
            //pose_to_optimize_ = optimized_key_frames_.at(optimized_index).pose * all_key_frames_.at(all_index).pose.inverse();//T = Tm_lop * Tl_m
            pose_to_optimize_ = all_key_frames_.at(all_index).pose.inverse() * optimized_key_frames_.at(optimized_index).pose;//Tl_lop = Tl_m * Tm_lop
    ...
    while (all_index < all_key_frames_.size()) {
        //all_key_frames_.at(all_index).pose = pose_to_optimize_ * all_key_frames_.at(all_index).pose; //T =T * Tm_l
        all_key_frames_.at(all_index).pose = all_key_frames_.at(all_index).pose * pose_to_optimize_; //T =Tm_l * Tl_lop
        all_index ++;
    }
