...
bool BackEndFlow::UpdateBackEnd() {
    ...
    if (!odometry_inited) {
        ...
        odom_init_pose = current_gnss_pose_data_.pose * current_laser_odom_data_.pose.inverse();//Tm_o = Tm_l * Tl_o
    }
    current_laser_odom_data_.pose = odom_init_pose * current_laser_odom_data_.pose;//Tm_l = Tm_o * To_l
