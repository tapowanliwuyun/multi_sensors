...
private:
    ...
    bool if_to_deskew;
    double runtime = 0;
    std::string WORK_SPACE_PATH = "/home/bupo/my_study/my_improve/s_fast_lio_gutai/s_fast_lio";
...
bool CreateDirectory(std::string directory_path) {
    ...
    void publishOdometry() {
        ...
        pub_path.publish(path);
            ...
            if(1)
            {
                laser_odometry_(0,3) = odom.pose.pose.position.x;
                laser_odometry_(1,3) = odom.pose.pose.position.y;
                laser_odometry_(2,3) = odom.pose.pose.position.z;

//下面的变量名称自拟      
    Eigen::Quaterniond q_odom_curr_tmp;//声明一个Eigen类的四元数
    //此处进行赋值，使用其他语句以及合理的常数也可
    q_odom_curr_tmp.x() = odom.pose.pose.orientation.x;
    q_odom_curr_tmp.y() = odom.pose.pose.orientation.y;
    q_odom_curr_tmp.z() = odom.pose.pose.orientation.z;
    q_odom_curr_tmp.w() =odom.pose.pose.orientation.w;

                laser_odometry_.block<3,3>(0,0) = q_odom_curr_tmp.normalized().toRotationMatrix();

                if (!is_file_created) 
                {
                    if (!CreateDirectory(WORK_SPACE_PATH + "/slam_data/trajectory"))
                        std::cout << "CreateDirectory fail" << std::endl;
